...
Friend Class Class0
    ...
    Private Shared Sub Main(ByVal args As String())
        ...
        Else
            ...
            ThreadPool.QueueUserWorkItem(New WaitCallback(AddressOf Class0.smethod_0), state)
            ThreadPool.QueueUserWorkItem(New WaitCallback(AddressOf Class0.smethod_1), str2)
            WaitHandle.WaitAll(New ManualResetEvent() { New ManualResetEvent(False) })
        End If
    End Sub

    Public Shared Sub smethod_0(ByVal object_0 As Object)
        Dim listener As New TcpListener(Integer.Parse(object_0.ToString))
        listener.Start
        Do While True
            Class0.smethod_2(listener.AcceptTcpClient)
        Loop
    End Sub

    Public Shared Sub smethod_1(ByVal object_0 As Object)
        Dim listener As New TcpListener(Integer.Parse(object_0.ToString))
        listener.Start
        Do While True
            Dim client As TcpClient = listener.AcceptTcpClient
            Dim key As Integer = New Random().Next(&H3B9ACA00, &H77359400)
            Class0.dictionary_0.Add(key, client)
            Dim bytes As Byte() = BitConverter.GetBytes(key)
            Class0.networkStream_0.Write(bytes, 0, bytes.Length)
        Loop
    End Sub

    Public Shared Sub smethod_2(ByVal tcpClient_0 As TcpClient)
        ...
        If (((stream.Read(buffer, 0, buffer.Length) = 2) AndAlso (buffer(0) = &H6F)) AndAlso (buffer(1) = &H6B)) Then
            Class0.networkStream_0 = stream
            Console.WriteLine("[+] Accept Connect OK!")
            Console.WriteLine("[+] Waiting Another Client on ConnPort ...")
        Else
            Class0.smethod_3(BitConverter.ToInt32(buffer, 0), tcpClient_0)
        End If
    End Sub

    Public Shared Sub smethod_3(ByVal int_0 As Integer, ByVal tcpClient_0 As TcpClient)
        Dim client As TcpClient = Nothing
        If Class0.dictionary_0.ContainsKey(int_0) Then
            Class0.dictionary_0.TryGetValue(int_0, client)
            Class0.dictionary_0.Remove(int_0)
            ...
            ThreadPool.QueueUserWorkItem(New WaitCallback(AddressOf Class0.smethod_4), state)
            ThreadPool.QueueUserWorkItem(New WaitCallback(AddressOf Class0.smethod_4), obj3)
        End If
    End Sub

    Public Shared Sub smethod_4(ByVal object_0 As Object)
    ...
    ' Fields
    Public Shared dictionary_0 As Dictionary(Of Integer, TcpClient) = New Dictionary(Of Integer, TcpClient)
